...
#4. to exclude genes that show up as having a noSTOP polymorphism because they might be truncated version in the reference.
...
#get noSTOP info:
f_nostop = open("/N/slate/pjohri/Paramecium/CNSGenomes/noSTOP_analysis/" + species + "_noSTOP_genes.char", 'r')
d_nostop = {}
for  line in f_nostop:
	line1 = line.strip('\n')
	line2 = line1.split('\t')
	if line2[0] != "gene":
		d_nostop[line2[0]] = "nostop"
f_nostop.close()
...
result = open("/N/slate/pjohri/Paramecium/CNSGenomes/" + species + "/all_genes_filtered_exclude_noSTOP.char", 'w+')
...
for line in f_char:
	...
	if line2[0] == "gene":
		...
		gene = line2[0]
		if d_nostop.get(gene, "NA") != "nostop":
			result.write(gene + '\t')
