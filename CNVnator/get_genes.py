...
result = open("/N/dc2/scratch/pjohri/Paramecium/CNVnator/" + species + "/" + species + ".genes.cnv", 'w+')
#d_len_affected = {}
...
for line in f_cnv:
	...
	line2 = line1.split('\t')
	if line2[0] == "Sample":
		result.write(line1 + '\t' + "gene;fraction_affected;deleted_position" + '\n')
	else:
		result.write(line1)
		...
		while start <= end:
			...
			if gene != "NA":
				d_len_affected[gene] = d_len_affected.get(gene, 0) + 1
		...
		for x in l_genes:
			...
			result.write('\t' + x + ';' + str(d_len_affected[x]) + ";" + str(float(d_len_affected[x])/float(d_len[x])))
		result.write('\n')
